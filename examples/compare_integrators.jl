@everywhere begin
    ...
    function run_integrator(dim, integrand, xmin, xmax, difficulty, nb_evals, integrator, nb_runs, nb_evals_gt)
        ...
        for _ = 1:nb_runs
            ...
            ground_truth = Integrator.mc(rand_integrand, xmin, xmax, nb_evals_gt)
            # compute mae sum
    ...
    function run_full(integrand, xmin, xmax, difficulty, list_nb_evals, nb_runs, nb_evals_gt, integrand_name, outdir)
        ...
        run_plot(dim, integrand, xmin, xmax, difficulty, list_nb_evals, Integrator.mc, nb_runs, nb_evals_gt, "mc")
        run_plot(dim, integrand, xmin, xmax, difficulty, list_nb_evals, Integrator.str, nb_runs, nb_evals_gt, "str")
        run_plot(dim, integrand, xmin, xmax, difficulty, list_nb_evals, Integrator.ucb, nb_runs, nb_evals_gt, "ucb")
        # finish plot
        xscale("log")
        yscale("log")
        grid("on")
...
# parallel runs
aux_data = [# (Integrand.sinus, fill(-8.0, 2), fill(8.0, 2), 1.0, "sinus"),
            # (Integrand.sinusc, fill(-8.0, 2), fill(8.0, 2), 1.0, "sinusc"),
            # (Integrand.sphere, fill(-2.0, 2), fill(2.0, 2), 1.0, "sphere"),
            # (Integrand.oscillatory, fill(-5.0, 2), fill(5.0, 2), 6.0, "oscillatory"),
            # (Integrand.productpeak, fill(-4.0, 2), fill(4.0, 2), 18.0, "productpeak"),
            # (Integrand.cornerpeak, fill(0.0, 2), fill(1.0, 2), 2.2, "cornerpeak"),
            (Integrand.gaussian, fill(-4.0, 2), fill(4.0, 2), 15.2, "gaussian"),
            # (Integrand.continuous, fill(-4.0, 2), fill(4.0, 2), 16.1, "continuous"),
            # (Integrand.discontinuous, fill(-4.0, 2), fill(4.0, 2), 16.4, "discontinuous"),
           ]
